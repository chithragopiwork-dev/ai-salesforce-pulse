...
function resolveFieldMap(
  ...
  return {
    ...
    /* FIXED HERE */
    name: find("Risk_Name__c", "Name", "risk name") || "Name",

    project:
      find("Project__c", "project", "Project_Name__c") || "Project__c",

    probability:
      find("Probability__c", "probability") || "Probability__c",

    impact:
      find("Impact__c", "impact") || "Impact__c",

    riskScore:
      find("Risk_Score__c", "risk score", "score") || "Risk_Score__c",

    owner:
      find("Owner__c", "Risk_Owner__c", "owner") || "Owner__c",

    deadline:
      find("Deadline__c", "deadline", "Due_Date__c") || "Deadline__c",

    mitigation:
      find("Mitigation_Plan__c", "mitigation") ||
      "Mitigation_Plan__c",
  };
}
...
export default function Risks() {
  const [fieldMap, setFieldMap] = useState<
    ReturnType<typeof resolveFieldMap> | null
  >(null);
  ...
  const sorted = [...risks].sort((a: any, b: any) => {
    ...
    const scoreA = a[fieldMap?.riskScore ?? "Risk_Score__c"];
    const scoreB = b[fieldMap?.riskScore ?? "Risk_Score__c"];

    return (order[scoreA] ?? 4) - (order[scoreB] ?? 4);
  });
    ...
    <div className="space-y-5 max-w-6xl">
      ...
      <div className="flex gap-2 flex-wrap">
        {["Critical", "High", "Medium", "Low"].map((s) => (
          <span
            key={s}
            className={`px-2.5 py-1 rounded-md text-xs font-medium ${countColors[s]}`}
          >
            {s}: {counts[s] ?? 0}
          </span>
        ))}
      ...
      {error && (
        <Card className="border border-red-200 bg-red-50">
          <CardContent className="p-4 text-sm text-red-700">
            Salesforce connection error: {(error as Error).message}
          </CardContent>
        </Card>
      )}
          ...
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-xs text-muted-foreground">
                {risks.length === 0
                  ? "No data returned from Salesforce."
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-[11px] hidden xl:table-cell">Mitigation</TableHead>
                </TableRow>
              </TableHeader>
              ...
              <TableBody>
                {filtered.map((r: any) => (
                  <TableRow key={r.Id}>
                    ...
                    <TableCell>
                      {scoreBadge(r[fieldMap!.riskScore])}
                    </TableCell>
